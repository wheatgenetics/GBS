...
# Version:  0.1 Sept 9,2014     Initial version based on blank_dna_quant_report.py
#
#
...
try:
    for plate in platelibs:
        ...
        query1 = ("SELECT plate_id,sample_name,sample_id,plate_name,well_A01 FROM dna WHERE plate_id LIKE %s and sample_name LIKE %s")
        cursor1.execute(query1, (plate_id_input, blank))
        if cursor1.rowcount != 0:
            for (plate_id, sample_name, sample_id, plate_name, well_A01) in cursor1:
                ...
                if cursor2.rowcount != 0:
                    for (sample_id, quant_val) in cursor2:
                        ...
                        report_list.append(
                            [plate_id, sample_name, plate_name, dna_sample_id, well_A01, dnaQuant_sample_id, quant_val])
                else:
                    report_list.append([plate_id, sample_name, plate_name, dna_sample_id, well_A01, 'Null', 0.0])
        else:
...
if report_list:
    try:
        ...
        with open(outputfile, 'wb') as csvfile:
            ...
            header.writerow(
                ['Plate ID', 'Sample Name', 'Plate Name', 'DNA Sample ID', 'Well A01', 'DNA Quant Sample ID',
                 'Quant Value'])
        csvfile.close
        ...
        with open(outputfile, 'ab') as csvfile:
            for line_item in report_list:
                ...
                reportline.writerow(
                    [line_item[0], line_item[1], line_item[2], line_item[3], line_item[4], line_item[5], line_item[6]])
    except IOError as e:
