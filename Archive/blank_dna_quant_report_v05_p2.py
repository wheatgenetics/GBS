...
# Version:  0.5 Sept 9,2014    Added ability to input comma separated list of DNA plate IDs
#                              Removed on-screen results table output.
    ...
    for gbs in gbslibs:
        ...
        if cursor1.rowcount!=0:
            for (gbs_id,dna_id,plexing) in cursor1:
                ...
                query2 = ("SELECT plate_id,sample_name,sample_id,plate_name,well_A01 FROM dna WHERE plate_id = %s and sample_name LIKE %s")
                cursor2.execute(query2,(dna_id,blank))
                if cursor2.rowcount!=0:
                    for (plate_id,sample_name,sample_id,plate_name,well_A01) in cursor2:
                        ...
                        if cursor3.rowcount!=0:
                            for (sample_id,quant_val) in cursor3:
                                ...
                                report_list.append([gbs_id,gbs_plexing,plate_id,sample_name,plate_name,dna_sample_id,well_A01,dnaQuant_sample_id,quant_val])
                        else:
                            report_list.append([gbs_id,gbs_plexing,plate_id,sample_name,plate_name,dna_sample_id,well_A01,'Null',0.0])
                else:
                    report_list.append([gbs_id,gbs_plexing,'Null','Null','Null','Null','Null','Null',0.0])
        else:
    ...
    try:
        ...
        with open(outputfile, 'ab') as csvfile:
            for line_item in report_list:
                ...
                reportline.writerow([line_item[0],line_item[1],line_item[2],line_item[3],line_item[4],line_item[5],line_item[6],line_item[7],line_item[8]])
    except IOError as e:
